...
class TwoFactorListBoxRow(Gtk.ListBoxRow):
    ...
    def __init__(self):
        ...
        for _index, name in enumerate([self.delete_button, self.edit_button, self.move_up_button, self.move_down_button]):
            name.get_style_context().add_class("circular")
        # add avatar to very start of row & set it's size
        self.row_hbox.pack_start(self.avatar, False, False, 6)
...
class ImageFromSvgResource(Gtk.Image):

    def __init__(resoutrce):

        # width, height and aspect ratio preservation
        width = 128
        height = 128
...
class EmptyListWidget(Gtk.Box):
    """
    A placeholder widget used in the list box when nothing else
    is present.
    """
    ...
    def __init__(self):
        ...
        image = self.image_from_svg_resource('/com/niallasher/twofactor/icon.svg',
                                        128,
                                        128,
                                        True)
        ...
        cvbox.pack_start(image, False, False, 6)
        cvbox.pack_start(empty_list_label, True, False, 6)
        vbox.set_center_widget(cvbox)
        self.pack_start(vbox, True, True, 3)
        self.show_all()

    def image_from_svg_resource(self,
                                resourcepath,
                                width,
                                height,
                                keepaspect = True) -> Gtk.Image:
        """
        Loads an SVG from GResources, converts it into an image,
        and returns it.
        """
        pixbuf = GdkPixbuf.Pixbuf.new_from_resource_at_scale(resourcepath,
                                                         width,
                                                         height,
                                                         keepaspect)
        image = Gtk.Image()
